...
public final class Resolver: ResolverProtocol {
	...
	public func register(_ containerScope: String, with serviceBuilder: (ContainerProtocol) -> Void) {
		lock.withLock {
			if let existingContainer = containers.first(where: { $0.scope == containerScope }) {
				serviceBuilder(existingContainer)
			} else {
				let container: any ContainerProtocol = Container(scope: containerScope)
	...
	public func resolve<T>(_ type: T.Type, name: String?) -> T {
		let container: (any ContainerProtocol)? = lock.withLock {
			containers.first { $0.contains(type, name: name) }
		}
